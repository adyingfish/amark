...
**Browse Skills** — most agent skills are written in plain Markdown, which makes them a natural fit for reading as one whole directory. Open a skills directory in AMark, follow the file tree to see where each skill's boundaries are, and jump along the references to the exact reference doc.
